...
//
// WriteTimeout covers the whole response. A synthesis run is allowed synthRunLimit to
// produce it, so a WriteTimeout at or below synthRunLimit does not stop attacks:
// it cuts off legitimate slow syntheses, and it does so intermittently, only on
...
func TestWriteTimeoutOutlastsTheSynthesisBudget(t *testing.T) {
	if srvWriteTimeout <= synthRunLimit {
		t.Fatalf("srvWriteTimeout = %s, synthRunLimit = %s: the server would abort its own\nresponse while the synthesis is still within its budget", srvWriteTimeout, synthRunLimit)
	}

	// Room to write the response after the synthesis has used its full budget. A
	...
	if margin := srvWriteTimeout - synthRunLimit; margin < minMargin {
		t.Errorf("margin over synthRunLimit = %s, want at least %s", margin, minMargin)
	}
}
